...
def _make_synthetic_sequence(n_frames: int = 400, seed: int = 0):
    """Synthetic per-frame lowest-body-point height, with a KNOWN floor and
    a known injected penetration event, for validating the detection logic
    end to end (no smplx/torch needed).

    The injected window directly OVERRIDES the height (rather than adding a
    constant offset on top of the natural gait-swing curve). This matters:
    an additive offset only crosses the penetration threshold during the
    part of the gait cycle that was already near the floor, which makes
    "did we recover the injected window" an ill-posed question. An
    override models what an actual corruption looks like -- the character
    is embedded in the floor for those frames, independent of gait phase --
    which is the same reasoning inject_synthetic_anomalies.py in this repo
    already applies to orientation corruptions (compose an SO(3)
    perturbation with an exactly known effect, rather than add noise whose
    detectability depends on the surrounding signal).
    """
    ...
    inj_start, inj_end = 150, 161                            # deliberate penetration event: ~5 cm below floor throughout
    min_height[inj_start:inj_end + 1] = true_floor - 0.05 + rng.normal(0, 0.003, size=inj_end - inj_start + 1)

    min_height[300] = true_floor - 0.005                     # sub-threshold dip: should NOT be flagged at the 2 cm default
...
def self_test(penetration_threshold_m: float = 0.02, floor_percentile: float = 5.0,
    ...
    inj_s, inj_e = injected_event
    inj_frames = set(range(inj_s, inj_e + 1))
    detected_frames = set()
    for s, e in events:
        detected_frames |= set(range(s, e + 1))
    coverage = len(inj_frames & detected_frames) / len(inj_frames)      # recall on the injected window
    extra_frames = detected_frames - inj_frames                         # flagged frames outside the injected window
    no_spurious = len(events) == 1 and len(extra_frames) == 0
    hit = coverage >= 0.9
    ...
    if verbose:
        ...
        print(f"  coverage of injected window: {coverage * 100:.1f}% ({'OK' if hit else 'FAIL'}, need >=90%)")
        print(f"  no spurious extra frames : {'YES' if no_spurious else 'NO'} (extra: {sorted(extra_frames)})")
